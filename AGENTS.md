...
- Keep the opening hierarchy simple. After the title, meta, and lead, use plain paragraphs before the first image or section; avoid placing a large callout immediately after the intro because it creates too many competing visual levels.
- Use callouts sparingly for true thesis statements. If a line only summarizes a sequence or operating path, use a compact inline note style instead of the large serif callout treatment.
- Article illustrations should support the reading rhythm, not dominate it. Use responsive figures that are centered and narrower than the text column on desktop, while staying full width on mobile. Avoid full-bleed or oversized images inside regular posts.
- Tables should stay visually inside the article reading column on desktop. Do not widen tables beyond the post width just to avoid wrapping; instead tune column widths, keep short first-column labels unwrapped, and let tables scroll horizontally inside their wrapper on mobile.
- A table of contents is acceptable only as a quiet desktop aid: narrow left rail, small text, sticky, current-section highlight. Hide it on mobile. Do not use a large sticky presentation-style TOC or make the article feel like a documentation site.
- When adding side rails or table wrappers, set the main article column to `min-width: 0` so tables and images do not force mobile overflow.
- After layout changes, check the first viewport and the changed section specifically. Look for mixed visual systems in one screen, oversized callouts, tables escaping the text column, images that overpower text, and mobile horizontal overflow.
...
- If `git push origin main` hangs with no output, first check whether GitHub is reachable: `curl -I --max-time 20 https://github.com` and `git ls-remote --heads origin main`. If both fail, treat it as a network/proxy/VPN problem and do not keep retrying the same push.
- Use bounded Git network commands when retrying, so a bad connection does not hang indefinitely: `git -c http.lowSpeedLimit=1 -c http.lowSpeedTime=10 push -v origin main`.
- If push fails with `fetch first`, fetch and compare before pushing: `git -c http.lowSpeedLimit=1 -c http.lowSpeedTime=10 fetch origin main`, then `git log --oneline --left-right HEAD...origin/main`.
- If `origin/main` was force-updated or is a snapshot commit and local `main` has diverged, do not force push the local branch. Create a temporary worktree from `origin/main`, cherry-pick the local blog commit, resolve conflicts by keeping remote content plus the new post, then push `HEAD:main`.
- Known safe recovery path for this repo:

```bash
git -c http.lowSpeedLimit=1 -c http.lowSpeedTime=10 fetch origin main
git worktree add /private/tmp/myblog-push-retry origin/main
cd /private/tmp/myblog-push-retry
git cherry-pick <local-post-commit>
git -c http.lowSpeedLimit=1 -c http.lowSpeedTime=10 push -v origin HEAD:main
cd /Users/gaoshimeng/Documents/20_Personal_Projects/myblog
git worktree remove /private/tmp/myblog-push-retry
```

- After a successful push, verify the exact remote commit with `git ls-remote --heads origin main`, then check the GitHub Pages homepage and changed article URL with `curl -I`.
